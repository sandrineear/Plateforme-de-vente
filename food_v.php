<?php 
	...
	$id = $idm->getUid();
    $res = $db -> prepare ('SELECT * FROM produits WHERE ctid = 1 AND uid ='.$id.'');
    $res->execute(['id'=>$id]);
    require ('include/header_ven.php');

    if(isset($_GET['q']) AND !empty($_GET['q'])){
        $q = htmlspecialchars($_GET['q']);
        $res = $db->prepare('SELECT * FROM produits WHERE nom LIKE "%'.$q.'%" AND uid ='.$id.' AND ctid = 1');
        $res->execute(['id'=>$id]);
    }
?>

<p class = "titre"> Alimentaires </p>
<div class ="container">
    <div class="row">
